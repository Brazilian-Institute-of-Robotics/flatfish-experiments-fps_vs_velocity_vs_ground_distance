...
// pocolog_cpp::InputDataStream* points_3d_stream =;
...
//   float skip_frames = 5;
//
//   std::string path = "/home/trocoli/dev.bir/bundles/flat_fish/logs/20170422-1843.1/";
//   std::string file_name = "stereo_point_cloud.log";
//   std::string stream_name ="stereo_camera_front.points_3d";
//
//   std::cout<< " passou aqui 1 " << std::endl;
//
//   pocolog_cpp::InputDataStream* points_3d_stream = NULL;
//   std::cout<< " passou aqui 2 " << std::endl;
//
//   // PocologTools::getDataStreamFromLog(path+file_name, stream_name);
//
//   pocolog_cpp::LogFile log_file(path+file_name);
//
//   std::cout<<"out 2" << std::endl;
//   points_3d_stream = dynamic_cast<pocolog_cpp::InputDataStream *>
...
//
//
//   base::samples::Pointcloud rock_cloud;
//   for (size_t i = 0; i < 1150 ; i++) {
//     rock_cloud = PocologTools::getSampleFromDataStream <base::samples::Pointcloud>
//                     (points_3d_stream, i);
//
//     std::cout<< i << " -> size cloud = "<< rock_cloud.points.size() << std::endl;
//     pcl::PointCloud<pcl::PointXYZRGB>::Ptr pcl_cloud (new pcl::PointCloud<pcl::PointXYZRGB>);
//
//     for (size_t j = 0; j < rock_cloud.points.size(); j++) {
//       uint8_t r = (rock_cloud.colors[j][0]*255);
//       uint8_t g = (rock_cloud.colors[j][1]*255);
//       uint8_t b = (rock_cloud.colors[j][2]*255);
//
//       pcl::PointXYZRGB pcl_point(r,g,b);
//
//       pcl_point.x = rock_cloud.points[j][0];
//       pcl_point.y = rock_cloud.points[j][1];
//       pcl_point.z = rock_cloud.points[j][2];
//
//       // pcl_cloud->push_back(pcl_cloud);
//
//     }
//
//
...
BOOST_AUTO_TEST_CASE(takeImges_testCase) {
  ...
  pocolog_cpp::InputDataStream* out_stream = NULL;
  std::cout<< " passou aqui 2 " << std::endl;

  pocolog_cpp::LogFile log_file(path+file_name);

  out_stream = dynamic_cast<pocolog_cpp::InputDataStream *>
                     (&(log_file.getStream(stream_name)));
